...
const GlobalWorkforceMap: React.FC = () => {
  ...
  const continents = [
    ...
    { id: 'north-america', name: 'North America', color: 'bg-brand-blue', icon: '🌎' },
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
          {continents.map((continent) => (
            <motion.div
              ...
              className={`continent-card rounded-lg shadow-md cursor-pointer p-2 text-center ${continent.color} ${selectedContinent === continent.id || (continent.id === 'all' && selectedContinent === null) ? 'ring-2 ring-white ring-opacity-60 shadow-lg' : ''}`}
            >
              <div className="text-xl mb-1">{continent.icon}</div>
              <h3 className="text-white text-xs font-semibold">{continent.name}</h3>
            </motion.div>
          ))}
        </div>
        ...
        >
          ...
          >
            ...
            {(selectedContinent === null || selectedContinent === 'africa') && africaLocations.map((location) => (
              <Marker 
                key={location.id} 
            ...
            {/* US Markers */}
            {(selectedContinent === null || selectedContinent === 'north-america') && usLocations.map((location) => (
              <Marker 
                key={location.id} 
                position={location.position}
                eventHandlers={{
            ...
            {/* Asia Markers */}
            {(selectedContinent === null || selectedContinent === 'asia') && asiaLocations.map((location) => (
              <Marker 
                key={location.id} 
                position={location.position}
                eventHandlers={{
            ...
            {/* Europe Markers */}
            {(selectedContinent === null || selectedContinent === 'europe') && europeLocations.map((location) => (
              <Marker 
                key={location.id} 
                position={location.position}
                eventHandlers={{
            ...
            {/* Caribbean Markers */}
            {(selectedContinent === null || selectedContinent === 'caribbean') && caribbeanLocations.map((location) => (
              <Marker 
                key={location.id} 
                position={location.position}
                eventHandlers={{
        ...
        <div className="text-center mt-8">
          <p className="text-gray-600 text-sm">
            {selectedContinent === null ? 
              ...
              `Showing locations in ${continents.find(c => c.id === selectedContinent)?.name}. Click on markers for details.`}
          </p>
        </div>
